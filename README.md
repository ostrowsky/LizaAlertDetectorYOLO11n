# New Detection Model for Liza Alert
...
Полученное значение mAP@50 = 0.976 говорит о том, что модель почти в 98% случаев правильно выявляет человека на тайле при допустимом пороге перекрытия 50%.
...
Для объединения результатов применяется масштабирование координат боксов в исходный размер и нелокальное подавление пересечений (Non-Maximum Suppression, NMS) для удаления дубликатов на границах тайлов. 
...
Для наглядности приведём сравнение финальных метрик на валидационном наборе тайлов для старой и новой моделей:
...
Как видно из таблицы, наша модель YOLO11n заметно превосходит YOLOv5 по качеству: при допустимом пороге 50% точность почти на 3% выше, а по сложной составной метрике COCO – выше на ~15% абсолютных. 
